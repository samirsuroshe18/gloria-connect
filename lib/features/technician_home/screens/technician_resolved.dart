...
import 'package:gloria_connect/features/technician_home/widgets/resolved_complaint_card.dart';
...
class _TechnicianResolvedState extends State<TechnicianResolved> {
  ...
  void initState() {
    super.initState();
    context.read<TechnicianHomeBloc>().add(GetResolvedComplaint());
  }
    ...
    return Scaffold(
      body: BlocConsumer<TechnicianHomeBloc, TechnicianHomeState>(
        listener: (context, state){
          if(state is GetResolvedComplaintLoading){
            _isLoading = true;
            _isError = false;
          }
          if(state is GetResolvedComplaintSuccess){
            _isLoading = false;
            _isError = false;
            data.clear();
            data.addAll(state.response);
          }
          if(state is GetResolvedComplaintFailure){
            data.clear();
            _isLoading = false;
            _isError = true;
              ...
              child: AnimationLimiter(
                child: ListView.builder(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    return StaggeredListAnimation(index: index, child: ResolvedComplaintCard(complaint: data[index]));
                  },
                ),
              ),
            );
