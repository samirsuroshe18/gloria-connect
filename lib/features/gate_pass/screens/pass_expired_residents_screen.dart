...
import 'package:gloria_connect/features/gate_pass/widgets/expired_gate_pass_card.dart';
import 'package:gloria_connect/common_widgets/staggered_list_animation.dart';
// ignore: depend_on_referenced_packages
import 'package:intl/intl.dart';

class PassExpiredResidentScreen extends StatefulWidget {
  const PassExpiredResidentScreen({super.key});

  @override
  State<PassExpiredResidentScreen> createState() => _PassExpiredResidentScreenState();
}

class _PassExpiredResidentScreenState extends State<PassExpiredResidentScreen> with AutomaticKeepAliveClientMixin {
  ...
  Future<void> _fetchEntries()async {
    ...
    context.read<GatePassBloc>().add(GatePassGetExpiredRes(queryParams: queryParams));
  }
    ...
    return Scaffold(
        body: BlocConsumer<GatePassBloc, GatePassState>(
          listener: (context, state) {
            if (state is GatePassGetExpiredResLoading) {
              _isLoading = true;
              _isError = false;
            }
            if (state is GatePassGetExpiredResSuccess) {
              if (_page == 1) {
                data.clear();
              }
            ...
            if (state is GatePassGetExpiredResFailure) {
              data = [];
              _isLoading = false;
              _isLazyLoading = false;
              _isError = true;
  ...
  Widget _buildGatePassList(){
    if (data.isNotEmpty && _isLoading == false) {
      ...
      return DataNotFoundWidget(onRefresh: _onRefresh, infoMessage: 'No expired Gate passes found', kToolbarCount: 4);
    }
  }
