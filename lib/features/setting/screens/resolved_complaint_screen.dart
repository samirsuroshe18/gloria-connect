...
class _ResolvedComplaintScreenState extends State<ResolvedComplaintScreen> with AutomaticKeepAliveClientMixin {
    ...
    if (_data.isNotEmpty && _isLoading == false) {
      return RefreshIndicator(
        ...
        child: AnimationLimiter(
          child: _buildEntriesList(complaints, context),
        ),
      );
  ...
  Widget _buildComplaintCard(BuildContext context, {required Complaint complaint}) {
    return Card(
      color: Colors.black.withOpacity(0.2),
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: InkWell(
        onTap: () => _cardOnTap(complaint, context),
  ...
  Widget _buildEntriesList(data, BuildContext context) {
    ...
    return ListView.builder(
        ...
        itemBuilder: (context, index) {
          if (index < data.length) {
            return StaggeredListAnimation(index: index, child: _buildComplaintCard(complaint: _data[index], context));
          } else {
            if (_hasMore) {
